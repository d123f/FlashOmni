...
def scaled_dot_attn_sparse(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, attention_mask: torch.Tensor = None):  
...
if __name__ == "__main__":
    ...
    for (B, H, N_tile, N_kv, D, spQ, spKV, sp_size) in BHNDs:
        ...
        print(f"sparse_info shape: {sparse_info.shape}, sparse_kv_info shape: {sparse_kv_info.shape}, text_token: {text_token}")
        ...
        # print("sparse ratio", torch.count_nonzero(attention_mask) / attention_mask.numel())
        ...
        # if args.check:
        #     out_scaled_dot,         throughput  = run_benchmark(scaled_dot_attn_sparse, q, k, v, tag="(scaled_dot)",  warmup=args.warmup, iters=args.iters, MAX_Throughput=max_tp)
        # else:
        #     _,                      throughput  = run_benchmark(scaled_dot_attn_sparse, q, k, v, tag="(scaled_dot)",  warmup=args.warmup, iters=args.iters, MAX_Throughput=max_tp)
        # max_tp                      = max(max_tp, throughput)
        # if args.check:
        ...
        # max_tp                      = max(max_tp, throughput)
        if args.check:
            out_scaled_dot_masked,  throughput  = run_benchmark(scaled_dot_attn_sparse, q, k, v, attention_mask=attention_mask, tag="(scaled_dot Masked)",  warmup=args.warmup, iters=args.iters, MAX_Throughput=max_tp)
        else:
            _,                      throughput  = run_benchmark(scaled_dot_attn_sparse, q, k, v, attention_mask=attention_mask, tag="(scaled_dot Masked)",  warmup=args.warmup, iters=args.iters, MAX_Throughput=max_tp)
        # max_tp                      = max(max_tp, throughput)
        # # FA2, SDPA official
        # if args.check:
        ...
        # # print(fiq.shape, fik.shape, fiv.shape, fiq.is_contiguous(), fik.is_contiguous(), fiv.is_contiguous())
        # if args.check:
        ...
        # max_tp                      = max(max_tp, throughput)
        # # sparse
        wrapper, fake_sparse_info, sparse_info_indptr, fake_sparse_kv_info, sparse_kv_info_indptr  = sparseqkv_mySparseKV(B, N, H, D, N_kv, causal=False, pos_encoding_mode="NONE", logits_soft_cap=0.0, sparse_size=sp_size)
        ...
        # if args.check:
        #     out_sparseqkv_sparse,   throughput  = run_benchmark(wrapper, fiq, fik, fiv, sparse_info=packed_sparse_info, 
        ...
        #     out_sparseqkv_sparse                = out_sparseqkv_sparse.view(B, N, H, D).transpose(1, 2).contiguous()
        # else:
        ...
        if args.check:
            out_sparseqkv_sparse_full,   throughput  = run_benchmark(wrapper, fiq, fik, fiv, sparse_info=packed_sparse_info, 
                                                            sparse_kv_info=packed_sparse_kv_info, 
                                                            sparse_info_indptr=sparse_info_indptr, 
                                                            sparse_kv_info_indptr=sparse_kv_info_indptr,
                                                            is_full=True,
                                                            tag="(sparse_full)",  warmup=args.warmup, iters=args.iters, MAX_Throughput=max_tp)
            out_sparseqkv_sparse_full                = out_sparseqkv_sparse_full.view(B, N, H, D).transpose(1, 2).contiguous()
        else:
            _,                      throughput  = run_benchmark(wrapper, fiq, fik, fiv, sparse_info=packed_sparse_info, 
                                                            sparse_kv_info=packed_sparse_kv_info, 
                                                            sparse_info_indptr=sparse_info_indptr, 
        ...
        if args.check:
            ...
            # check_all_close(out_unfused_masked, out_sparseqkv_sparse,        tag1="unfused",            tag2="sparseqkv",           is_flash=False)
            # check_all_close(out_scaled_dot_masked, out_sparseqkv_sparse,     tag1="scaled_dot_masked",  tag2="sparseqkv",           is_flash=False)
            check_all_close(out_flash, out_sparseqkv_sparse_full,     tag1="flash",  tag2="sparseqkv_full")
            check_all_close(out_scaled_dot, out_sparseqkv_sparse_full,                  tag1="scaled_dot",         tag2="sparseqkv_full",          is_flash=False)
            check_all_close(out_flash, out_sparseqkv_sparse_full,       tag1="flash",            tag2="sparseqkv_full")
            pretty_print_line()
