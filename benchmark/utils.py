...
def get_qkvo_global_sparse(B, H, N, N_kv, spq_Q=0.25, spq_KV=0.25, mode=1, sparse_size = 128, text_token = 512):
    ...
    # text_blocks = 512 / sparse_size
    text_blocks = text_token / sparse_size
    print(f"sparse_num: {sparse_num}, sparse_kv_num: {sparse_kv_num}, text_blocks: {text_blocks}")
    # spq_Q *= N/ N_kv
    ...
    # --- Generate the global index_info tensor ---
    # index_info is a 1D tensor of size total_elements
    index_info = torch.empty((total_elements,), device="cuda", dtype=torch.int32)

    # Flatten the sparse mask to calculate global kept indices
    flat_mask = sparse_Q_index.flatten() # Shape [total_elements]

    # Find the global 1D indices where the mask is 1 (globally kept indices)
    global_kept_indices = torch.nonzero(flat_mask, as_tuple=True)[0]

    # Total number of globally kept elements
    k_global = global_kept_indices.numel()

    # The logic for index_info[i] (now for the flattened index i):
    # For i < k_global (corresponding to the i-th kept element globally):
    #   index_info[i] = global_kept_indices[i] - i
    # This value represents the number of elements skipped *before* the i-th
    # globally kept element in the flattened view.
    if k_global > 0:
        # Calculate the index_info values for the first k_global positions
        # torch.arange(k_global) gives [0, 1, ..., k_global-1]
        index_info[:k_global] = global_kept_indices - torch.arange(k_global, device="cuda", dtype=torch.int32)

        # For i >= k_global, index_info[i] should be the total number of skips
        # in the entire flattened tensor.
        total_global_skipped = total_elements - k_global

        # Set the remaining index_info values to the total global skipped count
        index_info[k_global:] = total_global_skipped
    else:
        # If k_global is 0 (no elements kept globally), all index_info values
        # should be the total size (total_elements), ensuring all
        # index_info[i] + i results in values >= total_elements.
        index_info[:] = total_elements


    # sparse_Q_index remains B x H x sparse_num
    # index_info is a 1D tensor of size total_elements
    return sparse_Q_index, index_info
