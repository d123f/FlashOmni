 ...
 */
 #include <cstdint>
 #include <sparseqkv/gemm/w16a16/gemm_reduction.cuh>
 ...
 void SparseQKVGEMMReduction(at::Tensor x_ptr, at::Tensor w_ptr, at::Tensor y_ptr, 
   ...
   DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(x_ptr.scalar_type(), c_type, [&] {
     ...
     auto status = SparseQKVGEMMReductionRun<cutlass_t>(
     ...
     TORCH_CHECK(status == cudaSuccess,
           "Failed to run SparseQKVGEMMReduction: ", cudaGetErrorString(status));
     return true;
   });
