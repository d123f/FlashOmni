 ...
 */
 #include <cstdint>
 #include <sparseqkv/gemm/w16a16/gemm_reduction_quant.cuh>
...
#include "cute/numeric/numeric_types.hpp"
 ...
 void SparseQKVGEMMReductionQuant(at::Tensor x_ptr, at::Tensor w_ptr, at::Tensor y_ptr, std::optional<at::Tensor> bias, int64_t num_qo_heads, 
   ...
   // x_ptr.dtype();
  //  print("SparseQKVGEMMReductionQuantRun");
   ...
   DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(x_ptr.scalar_type(), c_type, [&] {
     ...
     auto status = SparseQKVGEMMReductionQuantRun<cutlass_t>(
         reinterpret_cast<cutlass_t*>(x_ptr.data_ptr()), reinterpret_cast<uint8_t*>(w_ptr.data_ptr()), 
     ...
     TORCH_CHECK(status == cudaSuccess,
           "Failed to run SparseQKVGEMMReductionQuant: ", cudaGetErrorString(status));
     return true;
   });
