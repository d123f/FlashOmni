 ...
 */
 #include <cstdint>
 #include <sparseqkv/gemm/w16a16/gemm_split.cuh>
 ...
 void SparseQKVGEMMSplit(at::Tensor x_ptr, at::Tensor w_ptr, at::Tensor y_ptr, 
                at::Tensor cached_hidden_ptr, std::optional<at::Tensor> bias, int64_t num_qo_heads, 
                at::Tensor sparse_info, at::Tensor sparse_info_indptr, int64_t num_text_tokens) {
   ...
   DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(x_ptr.scalar_type(), c_type, [&] {
     ...
     auto status = SparseQKVGEMMSplitRun<cutlass_t>(
         reinterpret_cast<cutlass_t*>(x_ptr.data_ptr()), reinterpret_cast<cutlass_t*>(w_ptr.data_ptr()), 
         reinterpret_cast<cutlass_t*>(y_ptr.data_ptr()), reinterpret_cast<cutlass_t*>(cached_hidden_ptr.data_ptr()), bias_ptr, 
     ...
     TORCH_CHECK(status == cudaSuccess,
           "Failed to run SparseQKVGEMMSplit: ", cudaGetErrorString(status));
     return true;
   });
