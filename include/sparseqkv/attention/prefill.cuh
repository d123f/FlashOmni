 ...
 */
#ifndef SPARSEQKV_PREFILL_CUH_
#define SPARSEQKV_PREFILL_CUH_
...
template <typename KTraits, typename Params>
__global__ __launch_bounds__(KTraits::NUM_THREADS) void BatchSparseFAWithRaggedKVKernel(
    const __grid_constant__ Params params) {
  using DTypeQ = typename Params::DTypeQ;
#if (__CUDA_ARCH__ < 800)
  if constexpr (std::is_same_v<DTypeQ, nv_bfloat16>) {
    SPARSEQKV_RUNTIME_ASSERT("Prefill kernels do not support bf16 on sm75.");
  } else {
#endif
...
cudaError_t BatchSparseFAWithRaggedKVDispatched(Params params, typename Params::DTypeO* tmp_v,
  ...
  int dev_id = 0;
  SPARSEQKV_CUDA_CALL(cudaGetDevice(&dev_id));
  int max_smem_per_sm = 0;
  SPARSEQKV_CUDA_CALL(cudaDeviceGetAttribute(&max_smem_per_sm,
  ...
  DISPATCH_NUM_MMA_KV(min(max_num_mma_kv_smem, max_num_mma_kv_reg), NUM_MMA_KV, {
    ...
    if constexpr (KTraits::IsInvalid()) {
      ...
      err_msg << "FlashInfer Internal Error: Invalid configuration : NUM_MMA_Q=" << NUM_MMA_Q
              ...
              << " please create an issue (https://github.com/sparseqkv-ai/sparseqkv/issues)"
                 " and report the issue to the developers.";
      SPARSEQKV_ERROR(err_msg.str());
    } else {
      size_t smem_size = sizeof(typename KTraits::SharedStorage);
      auto kernel = BatchSparseFAWithRaggedKVKernel<KTraits, Params>;
      SPARSEQKV_CUDA_CALL(
      ...
      if (tmp_v == nullptr) {
        ...
        SPARSEQKV_CUDA_CALL(
            cudaLaunchKernel((void*)kernel, nblks, nthrs, args, smem_size, stream));
        if constexpr (AttentionVariant::use_softmax) {
          SPARSEQKV_CUDA_CALL(VariableLengthMergeStates(
