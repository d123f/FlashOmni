 ...
 */
#ifndef SPARSEQKV_CASCADE_CUH_
#define SPARSEQKV_CASCADE_CUH_
...
cudaError_t MergeStates(DTypeIn* v, float* s, DTypeO* v_merged, float* s_merged,
  ...
  DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
    ...
    if (num_index_sets >= seq_len) {
      ...
      SPARSEQKV_CUDA_CALL(cudaLaunchKernel((void*)kernel, nblks, nthrs, args, smem_size, stream));
    } else {
      uint32_t bdy = num_heads;
      dim3 nblks(seq_len);
      dim3 nthrs(bdx, bdy);
...
cudaError_t VariableLengthMergeStates(DTypeIn* v, float* s, IdType* indptr, DTypeO* v_merged,
  ...
  DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
    ...
    SPARSEQKV_CUDA_CALL(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks_per_sm, kernel,
