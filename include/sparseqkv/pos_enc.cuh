 ...
 */
#ifndef SPARSEQKV_POS_ENC_CUH_
#define SPARSEQKV_POS_ENC_CUH_
...
cudaError_t BatchQKApplyRotaryPosIdsCosSinCache(
  ...
  SPARSEQKV_CUDA_CALL(cudaDeviceGetAttribute(&num_sms, cudaDevAttrMultiProcessorCount, dev_id));
  ...
  DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
    DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
      ...
      SPARSEQKV_CUDA_CALL(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      ...
      if ((nnz + bdy - 1) / bdy >= num_ctas_0) {
        dim3 nblks(nblks_x);
        dim3 nthrs(bdx, bdy);
        SPARSEQKV_CUDA_CALL(cudaLaunchKernel((void*)kernel_0, nblks, nthrs, args, 0, stream));
      } else {
        ...
        auto kernel_1 =
            BatchQKApplyRotaryPosIdsCosSinCacheHeadParallelismKernel<INTERLEAVE, HEAD_DIM, vec_size,
                                                                     bdx, DType, IdType>;
        SPARSEQKV_CUDA_CALL(cudaLaunchKernel((void*)kernel_1, nblks, nthrs, args, 0, stream));
      }
    });
  });
  ...
  DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
    DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
      ...
      void* args[] = {(void*)&q,
                      ...
                      (void*)&rope_rcp_scale,
                      (void*)&rope_rcp_theta};
      SPARSEQKV_CUDA_CALL(cudaLaunchKernel((void*)kernel, nblks, nthrs, args, 0, stream));
    });
  });
