 ...
 */
#ifndef SPARSEQKV_MMA_CUH_
#define SPARSEQKV_MMA_CUH_
...
#define SPARSEQKV_MMA_F8F8F32_M16N8K32_ENABLED
#endif
#endif

#if (__CUDACC_VER_MAJOR__ >= 11)
#if (!defined(__CUDA_ARCH__) || (__CUDA_ARCH__ >= 900))
#define SPARSEQKV_STMATRIX_M8N8X4_ENABLED
#endif
#if (!defined(__CUDA_ARCH__) || (__CUDA_ARCH__ >= 800))
#define SPARSEQKV_MMA_F16F16F32_M16N8K16_ENABLED
#define SPARSEQKV_MMA_F16F16F16_M16N8K16_ENABLED
#define SPARSEQKV_MMA_S8S8S32_M16N8K32_ENABLED
#endif
#if (!defined(__CUDA_ARCH__) || (__CUDA_ARCH__ >= 750))
#define SPARSEQKV_MMA_F16F16F32_M16N8K8_ENABLED
#define SPARSEQKV_MMA_F16F16F16_M16N8K8_ENABLED
#define SPARSEQKV_LDMATRIX_M8N8X4_ENABLED
#endif
#endif

#if defined(__CUDA_ARCH__)
#define SPARSEQKV_RUNTIME_ASSERT(x) __brkpt()
#else
#define SPARSEQKV_RUNTIME_ASSERT(x) assert(0 && x)
#endif
...
#ifdef SPARSEQKV_LDMATRIX_M8N8X4_ENABLED
  uint32_t smem_int_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(smem_ptr));
  asm volatile("ldmatrix.sync.aligned.m8n8.x4.shared.b16 {%0, %1, %2, %3}, [%4];\n"
               : "=r"(R[0]), "=r"(R[1]), "=r"(R[2]), "=r"(R[3])
               : "r"(smem_int_ptr));
#else
  SPARSEQKV_RUNTIME_ASSERT("Unsupported CUDA architecture for ldmatrix instruction");
#endif
}
...
#ifdef SPARSEQKV_STMATRIX_M8N8X4_ENABLED
  uint32_t smem_int_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(smem_ptr));
...
#if defined(SPARSEQKV_MMA_F8F8F32_M16N8K32_ENABLED)
...
__device__ __forceinline__ void mma_sync_m16n16k16_row_col_f16f16f32(float* C, uint32_t* A,
                                                                     uint32_t* B) {
#if defined(SPARSEQKV_MMA_F16F16F32_M16N8K16_ENABLED)
  if constexpr (mma_mode == MMAMode::kInit) {
...
#elif defined(SPARSEQKV_MMA_F16F16F32_M16N8K8_ENABLED)
  if constexpr (std::is_same_v<T, half>) {
    if constexpr (mma_mode == MMAMode::kInit) {
      asm volatile(
    ...
    SPARSEQKV_RUNTIME_ASSERT("Unsupported CUDA architecture for mma instruction");
  }
#else
  SPARSEQKV_RUNTIME_ASSERT("Unsupported CUDA architecture for mma instruction");
#endif
...
__device__ __forceinline__ void mma_sync_m16n16k16_row_col_f16f16f16(uint32_t* C, uint32_t* A,
                                                                     uint32_t* B) {
#if defined(SPARSEQKV_MMA_F16F16F16_M16N8K16_ENABLED)
  if constexpr (mma_mode == MMAMode::kInit) {
    asm volatile(
...
#elif defined(SPARSEQKV_MMA_F16F16F16_M16N8K8_ENABLED)
  if constexpr (mma_mode == MMAMode::kInit) {
    asm volatile(
 ...
 */
 ...
 __device__ __forceinline__ void mma_sync_m16n8k32_row_col_s8s8s32(int32_t* C, uint32_t* A,
                                                                    uint32_t* B) {
 #ifdef SPARSEQKV_MMA_S8S8S32_M16N8K32_ENABLED
