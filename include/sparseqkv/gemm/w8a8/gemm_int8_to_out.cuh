...
template <uint32_t CTA_M, uint32_t CTA_N, uint32_t CTA_K, uint32_t WARP_M, uint32_t WARP_N,
       uint32_t K_STAGE=2, bool has_bias=false, bool weight_asym=false, 
       ScaleMulMode scale_mul_mode=ScaleMulMode::kMode1, 
       GemmI8SparseMode gemm_i8_sparse_mode=GemmI8SparseMode::SMode3>
__global__ void sparseqkv_gemm_int8_to_out_kernel(const int8_t *__restrict__ A, const int8_t *__restrict__ B,
                              ...
                              uint8_t* sparse_info, uint32_t* sparse_info_indptr, uint32_t num_text_tokens)
{
  ...
  bool is_for_cached = (gemm_i8_sparse_mode == GemmI8SparseMode::SMode3);
  uint8_t* sparse_info_cur = sparse_info + sparse_info_indptr[batchIdx];
  // const uint32_t tile_per_head = head_dim / CTA_K;
...
#pragma unroll
  ...
  const uint32_t sparse_kv_info_offset_base = (num_text_tokens + blockIdx_m * CTA_M) / 128 * num_qo_heads;
  bool execute_bools[8];
  bool execute_this_tile;
  // prefetch K_STAGE stages of data
#pragma unroll
  for (; offset_K < K && cur_cached_stages < K_STAGE - 1;)
  // for (uint32_t stage = 0; stage < K_STAGE - 1; stage++)
  {
    
    sparse_info_offset = sparse_kv_info_offset_base + offset_K / head_dim;
...
#pragma unroll
    ...
    if constexpr (gemm_i8_sparse_mode == GemmI8SparseMode::SMode3) {
      // SMode3 means we need to load the data for the current tile
      execute_this_tile = !execute_bools[sparse_info_offset_mod_8];
    } else {
      // SMode1 and SMode2 means we need to load the data for the current tile
      execute_this_tile = execute_bools[sparse_info_offset_mod_8];
    }
  ...
  #pragma unroll
  for (int fake_commit = 0; fake_commit < K_STAGE - 1 - cur_cached_stages; fake_commit ++) {
      cp_async::commit_group();
  }
...
#pragma unroll
  ...
  const half *bias_warp_ptr = nullptr;
  if constexpr (gemm_i8_sparse_mode == GemmI8SparseMode::SMode3) {
    bias_warp_ptr = Bias + blockIdx_n * CTA_N + get_warp_idx_n<num_warps_m, num_warps_n>() * WARP_N;
  } else if constexpr (gemm_i8_sparse_mode == GemmI8SparseMode::SMode4) {
    // bias_warp_ptr = Bias + batchIdx * M * N + blockIdx_m * CTA_M * N + CTA_M / num_warps * warp_id * N + blockIdx_n * CTA_N;
    bias_warp_ptr = nullptr; // not used in SMode4
  }
...
cudaError_t SparseQKVGEMMInt8ToOutCachedRun(int8_t* input, int8_t* weight, half* output, half* bias, 
    ...
    auto kernel = sparseqkv_gemm_int8_to_out_kernel<CTA_M, CTA_N, CTA_K, WARP_M, WARP_N, K_STAGE, true, true, ScaleMulMode::kMode1, GemmI8SparseMode::SMode3>;
