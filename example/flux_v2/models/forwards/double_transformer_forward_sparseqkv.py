...
from ..taylorseer_utils import derivative_approximation, taylor_formula, taylor_cache_init, get_sparse_info, del_cache

def sparsetaylorseer_flux_double_block_forward(
    ...
    if current['type'] == 'full' or current['sparse_type'] == 'sparseqkv':
        current['module'] = 'attn'
        ...
        if current['sparseqkv'] and current['type'] == 'full' \
            and cache_dic['cache_index']['taylor_start'][current['stream']][current['layer']] == False:
            sparse_q_ratio, _ = get_sparse_info(cache_dic, current)
            if (sparse_q_ratio <= cache_dic['saving_threshold_q_for_taylor']):
                cache_dic['cache_index']['taylor_start'][current['stream']][current['layer']] = True
                del_cache(cache_dic=cache_dic, current=current)

        if len(attention_outputs) == 2:
            attn_output, context_attn_output = attention_outputs
        elif len(attention_outputs) == 3:
            attn_output, context_attn_output, ip_attn_output = attention_outputs
            raise NotImplementedError("Not implemented for SparseQKV yet.") 
    ...
    elif current['sparse_type'] == 'taylor_cache':
        current['module'] = 'attn'
        # Attention.
