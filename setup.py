...
head_dims = os.environ.get("SPARSEQKV_HEAD_DIMS", "64,128,256").split(",")
...
enable_aot = os.environ.get("SPARSEQKV_ENABLE_AOT", "0") == "1"
enable_f16 = os.environ.get("SPARSEQKV_ENABLE_F16", "1") == "1"
enable_bf16 = os.environ.get("SPARSEQKV_ENABLE_BF16", "1") == "1"
enable_int8 = os.environ.get("SPARSEQKV_ENABLE_INT8", "1") == "1"
enable_uint8 = os.environ.get("SPARSEQKV_ENABLE_UINT8", "1") == "1"
enable_fp8 = os.environ.get("SPARSEQKV_ENABLE_FP8", "1") == "1"
enable_fp8_e4m3 = (
    os.environ.get("SPARSEQKV_ENABLE_FP8_E4M3", "1" if enable_fp8 else "0") == "1"
)
enable_fp8_e5m2 = (
    os.environ.get("SPARSEQKV_ENABLE_FP8_E5M2", "1" if enable_fp8 else "0") == "1"
)
# enable_sm90 = os.environ.get("SPARSEQKV_ENABLE_SM90", "0") == "1"
enable_sm90 = False
...
def get_version():
    ...
    local_version = os.environ.get("SPARSEQKV_LOCAL_VERSION")
...
def generate_build_meta(aot_build_meta: dict) -> None:
    ...
    write_if_different(root / "sparseqkv" / "_build_meta.py", build_meta_str)
...
def generate_cuda() -> None:
    ...
    write_if_different(root / "sparseqkv" / "jit" /"aot_config.py", aot_config_str)
    write_if_different(
...
if enable_aot:
    ...
    if os.environ.get("SPARSEQKV_USE_CXX11_ABI"):
        # force use cxx11 abi
    ...
    if enable_f16:
        torch_cpp_ext.COMMON_NVCC_FLAGS.append("-DSPARSEQKV_ENABLE_F16")
    if enable_bf16:
        torch_cpp_ext.COMMON_NVCC_FLAGS.append("-DSPARSEQKV_ENABLE_BF16")
    if enable_int8:
        torch_cpp_ext.COMMON_NVCC_FLAGS.append("-DSPARSEQKV_ENABLE_INT8")
    if enable_uint8:
        torch_cpp_ext.COMMON_NVCC_FLAGS.append("-DSPARSEQKV_ENABLE_UINT8")
    if enable_fp8_e4m3:
        torch_cpp_ext.COMMON_NVCC_FLAGS.append("-DSPARSEQKV_ENABLE_FP8_E4M3")
    if enable_fp8_e5m2:
        torch_cpp_ext.COMMON_NVCC_FLAGS.append("-DSPARSEQKV_ENABLE_FP8_E5M2")
    ...
    kernel_sources = [
        ...
        "csrc/gemm_reduction_quant.cu",
        "csrc/gemm_split.cu",
        "csrc/gemm_int8.cu",
        "csrc/sparseqkv_ops.cu",
    ]
    ...
    ext_modules = [
        torch_cpp_ext.CUDAExtension(
            name="sparseqkv.sparseqkv_kernels",
    ...
    if enable_sm90:
        ext_modules += [
            torch_cpp_ext.CUDAExtension(
                name="sparseqkv.sparseqkv_kernels_sm90",
